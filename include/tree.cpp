...
template <class T>
auto BStree<T>::fillListOfNodes(list<const Root*> & listOfNodes, const Root* root) ->void
{
	if (root) {
		if (root->left != nullptr) {
			fillListOfNodes(listOfNodes, root->left);
		}

		listOfNodes.push_back(root);

		if (root->right != nullptr) {
			fillListOfNodes(listOfNodes, root->right);
		}
	}
}

template <class T>
auto BStree<T>::begin() const->iterator
{
	return iterator(root);
}

template <class T>
auto BStree<T>::end() const->iterator
{
	return iterator();
}

template <class T>
BStree<T>::NodeIterator::NodeIterator(const Root* root) : NodeIterator()
{
	fillListOfNodes(history_, root);
}

template <class T>
auto BStree<T>::NodeIterator::operator == (const NodeIterator & _iterator) -> bool
{
	cout << "asd";
...
template <typename T>
auto BStree<T>::NodeIterator::operator ++() -> NodeIterator
{
	history_.pop_front();
	return *this;
}
...
template <typename T>
auto BStree<T>::NodeIterator::operator *() const -> const T &
{
	return history_.front()->data;
}
